...
Fetch each candidate's full **description and issue links** via `getJiraIssue` (`fields: ["summary", "description", "status", "priority", "labels", "issuelinks"]`, `expand: "issuelinks"`). Both `issuelinks` in `fields` and `expand: "issuelinks"` are **mandatory** in this mode — without them, JIRA-native `is blocked by` links (the authoritative source for dependency ordering) are invisible to the agent and the cross-bucket abort in §2c cannot be enforced. If either parameter is missing or the API rejects them, **abort** with a setup pointer rather than producing a sequence based on description text alone. Read each ticket, then rank with the four axes below — applied lexicographically (axis 1 outranks axis 2, etc.) within the status weight bucket from §2c.

1. **Dependency graph** — combine two sources, with the JIRA link as authoritative:
   - **JIRA `Blocks`-type issue links** (authoritative; from the mandatory `issuelinks` fetch). JIRA links are directional, so the extraction rule must be precise. For each candidate ticket `T`, scan `T.fields.issuelinks` and collect blockers as follows:
     - If a link has `type.name == "Blocks"` and `inwardIssue.key` is present, then the inward key blocks `T` (i.e. `T` is blocked by `inwardIssue.key`). Add `inwardIssue.key` to `T`'s blocker set. Equivalent test: `type.inward == "is blocked by"`.
     - If a link has `type.name == "Blocks"` and `outwardIssue.key` is present, then `T` blocks the outward key. Record `T` as a blocker of `outwardIssue.key` (i.e. add `T` to that target's blocker set). Equivalent test: `type.outward == "blocks"`.
     - **Never** swap inward/outward. Treating a blockee as a blocker (the inverse direction) silently inverts the topological order and lets the apply path place dependent work ahead of its prerequisite.
     - Other link types (`Relates`, `Duplicate`, `Cloners`, etc.) are ignored for dependency ordering. They may inform §2c (cluster grouping) but not topological precedence.
   ...
   - **Out-of-scope blocker.** A blocker may live outside the current candidate set — past row 50, in a different epic via `--parent`, behind a `--status` filter, or in another project entirely. Reranking cannot move what it cannot see. For each out-of-scope blocker key, fetch its status via `getJiraIssue` (`fields: ["status"]` only). If the blocker's status is terminal (`Done` / `Closed` / `Resolved` / `Cancelled` — i.e. category `done`), the dependency is satisfied; ignore it. Otherwise the blockee cannot be safely reranked: **abort** with the out-of-scope blocker named and the scope-widening hint (drop `--parent`, raise the candidate cap, etc.). Listed as a hard-stop condition below. Never apply a rerank when an active prerequisite lives outside scope.
   - **Cross-bucket conflict.** If a blocker is in-scope but sits in a lower-priority status bucket than its blockee (e.g. a To Do prerequisite blocks an In Progress ticket), §2c forbids reordering across buckets — so the dependency invariant cannot be satisfied by reranking alone. Treat this as a workflow-state inconsistency: **abort the rerank** and report the conflicting pair with the suggested remediation (move the blockee back to a lower status, or move the blocker up). Listed as a hard-stop condition below. Never silently produce a sequence that violates the dependency invariant.
...
| LLM-judgement mode finds an out-of-scope blocker with non-terminal status (past row 50, behind `--parent` / `--status` / different project) | Abort, name the blocker key + its status + the blockee, suggest widening the scope (drop `--parent`, raise candidate cap, include the other project) or transitioning the blocker to terminal. Skip out-of-scope blockers whose status is `Done` / `Closed` / `Resolved` / `Cancelled` — they are not blocking. |
